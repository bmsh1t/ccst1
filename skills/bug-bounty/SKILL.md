---
name: bug-bounty
description: End-to-end bug bounty workflow for recon, pre-hunt learning, vulnerability hunting, chain building, validation, and report preparation. Covers web, API, cloud, AI/LLM, source-audit, CTF/lab, and VAPT-style tasks; routes to narrower skills for methodology, recon, vuln-class testing, payload reference, triage validation, and report writing. Use when a task spans multiple hunting phases or needs target isolation, impact-first reasoning, A-to-B chaining, or report readiness checks.
---

> **Live-action boundary for Claude CLI**: this broad skill includes historical
> bug-hunting examples that mention OTP/SMS, race, payment, coupon, wallet,
> cart, checkout, or order flows. Do not suppress those lanes by keyword; they
> are often high-value. In `/autopilot` and default `/hunt`, control only real
> side effects: persistent mutation, real money movement, irreversible lifecycle
> changes, or bulk external sends require explicit current-turn intent.

# Bug Bounty Master Workflow

Full pipeline: Recon -> Learn -> Hunt -> Validate -> Report. Use this as the broad workflow coordinator.

## 四层记忆接入

本 Skill 是端到端流程协调器。执行时遵守 `skills/runtime-protocol.md`：

1. 先读取目标层：`memory/goals/active.json` 或 `python3 tools/target_memory.py show`。
2. 本 Skill 负责路由到更窄 Skill，不默认全量读取知识库。
3. 需要发散思路时，由当前 Skill 按需读取 `knowledge/index.md` 和相关知识卡。
...
Use narrower skills first when the task is clearly scoped: `bb-methodology` for session route selection, `web2-recon` for recon, `web2-vuln-classes` for a specific bug class, `mobile-pentest` for Android/iOS app work, `cicd-security` for CI/CD and supply-chain workflow issues, `triage-validation` for finding gates, `report-writing` for report drafts, `security-arsenal` for payloads/bypasses, and the Web3/token skills for those domains. Return here for end-to-end flow, target isolation, bug chaining, and when the task spans multiple domains.

Do not let this broad skill override fresh target/session defaults, browser-first evidence collection, CTF/lab sandbox scope, or source-intel guidance from active commands, agents, and rules.
...
>
> If the answer is NO -- **do not report it yet.** During exploration, keep
> it only as a lead/signal when there is a concrete next evidence action;
> otherwise rotate.
...
**You must demonstrate actual harm before reporting. "Could" is not a
report-ready bug. Prove it works, keep it as a chain candidate with a next
evidence action, or drop it.**
...
When a bug class lives behind login, keep authenticated coverage active. IDOR,
BOLA, privilege escalation, mass assignment, JWT, GraphQL field-level auth,
auth-bypass validation, SSRF behind login, and many stateful app/AI bugs often
need one real session; IDOR/BOLA/priv-esc usually need two identities. Use
`session_id` / audit artifacts to diff attacker vs victim behavior. Reference:
`docs/auth-sessions.md` and `docs/auth.example.json`.

### Finding State Model

Use this state model. `rules/hunting.md` is canonical; this skill keeps the
execution reminder:
...
- **Lead**: plausible endpoint, source-intel hypothesis, anomaly, or chain
  seed with a next evidence action.
- **Signal**: observed behavior that might matter, but still needs replay,
  victim proof, or impact confirmation.
- **Candidate**: enough concrete evidence exists to run `/triage` or
  `/validate`.
- **Validated Finding**: passed the 7-Question Gate and all 4 pre-submission
  gates.
- **Report**: human-reviewed report draft or submission package.

The 7-Question Gate and 4 gates are pre-report/pre-submit controls. Do not use
them to erase raw hunt leads, hypotheses, anomalies, or chain seeds. Weak or
incomplete items are not report-ready; preserve, demote, or rotate them with a
specific next evidence action.
...
- Only the current user turn can set a temporary module skip for the current
  target.
- Do not inherit skipped bug classes, focus lanes, or “ignore this class”
  requests from previous targets, old Claude Code sessions, README examples, or
  non-resumed agent traces.
- In CTF/lab mode, do not add skips because a class is common or was skipped
  before.
- Production-looking brands, public-sector/government-style labels,
  account/login/register wording, account-gated surfaces, and old
  target-history caution notes are sandbox context, not lane exclusions.
...
parameter fuzzing, JS/config exposure discovery, and scanner discovery.
...
This broad skill coordinates the end-to-end hunt. It should not duplicate the
full session mindset and route-selection playbook. When the next question is
"what should I do next?", "where am I in the process?", "am I going wide or
deep?", or "why does this target feel stuck?", route to
`skills/bb-methodology/SKILL.md`.

Keep only these coordinator reminders here:

- Hunt the feature, not just the endpoint.
- Prioritize crown-jewel impact: tenant data, admin control, ATO, money,
  secrets, source/CI/CD control, or code execution.
- Payment, billing, wallet, order, coupon, and checkout lanes are valid
  high-value logic surfaces; avoid only real money movement, irreversible
  lifecycle changes, destructive mutation, or bulk external sends unless the
  current operator explicitly opts in.
- If a lane is dry, preserve useful leads and rotate by endpoint, role,
  version, source/browser evidence, or bug class instead of restarting recon.
...
6. **Race Windows in Non-Payment Quotas** -- check-then-consume as two DB operations = overuse
...
- [ ] Non-payment quota / invite / usage limits
- [ ] Voting / rating limits
- [ ] OTP / invite / quota race hypothesis, but do not brute-force or create high-pressure traffic

真实目标上不要直接提高并发或批量打请求。先按 `rules/red-lines.md`
判断是否会造成高压流量或状态改变；优先使用源码分析、单次 replay、
测试资源、dry-run 或本地复现说明 race 风险。没有明确授权和测试资源时，
只记录为 Lead 和下一步验证条件。

### Turbo Intruder -- Single-Packet Attack (All Requests Arrive Simultaneously)
```python
def queueRequests(target, wordlists):
    # Lab / explicitly authorized test resources only. Do not use this against
    # real target state without current-turn authorization and red-line review.
...
## Business Logic
- [ ] Review payment/cart/checkout/price logic as high-value surface; avoid real charge/refund/transfer unless explicitly intended
- [ ] Non-payment workflow skip
...
| ASI07 | Model DoS | Identify risk through local/model-lab reasoning or bounded evidence; do not cause token/CPU/OOM exhaustion on a live target |
...
All 7 must be YES before a report is written. Any NO -> stop the report path,
then either demote the item to Lead/Signal with a next evidence action or drop
it if no useful action remains.

### Q1: Can I exploit this RIGHT NOW with a real PoC?
Write the exact HTTP request. If you cannot produce a working request -> do not
report it yet.
...
Check the list below. If it's there and you can't chain it -> do not report it.
...
These items can still be chain seeds during exploration, but they are never
standalone reports until the chain is proven end-to-end.
...
Race condition in invite quota allows usage limit bypass
```
