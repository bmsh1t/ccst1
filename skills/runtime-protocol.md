...
- 需要跨来源经验（目标情节 + 跨目标技术经验 + 已晋升卡）时，用一个统一读入口：
  `python3 tools/experience_recall.py --target <target> [--query <词项>]`。该视图是
  当前目标 + 跨目标脱敏经验的聚合（目标隔离：其他目标的私有情节不混入）；
  query 只是词项粗筛，相关性判断由 Claude 读视图完成。向量索引暂缓：跨目标
  情节超过 ~500 条或视图超读取预算时再启动设计与 A/B 验收。
