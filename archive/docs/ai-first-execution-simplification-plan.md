...
已有 Wave 的实现、测试和提交状态以仓库中的实际记录为准；本方案不再把所有阶段
标记为已通过最终质量门。十次真实能力变更观测尚未开始，
因此不能宣称 Skill/Knowledge/Rules-only 比例、维护成本下降或边界提取目标已经达成。

后续按正常节奏开发功能：每次符合条件的能力变更记录变更类型、durable owner
触及情况、聚焦验证耗时和恢复结果；累计十次后再做一次只读复盘。没有真实数据
暴露具体边界时，不启动模块拆分或其他预防性重构，也不人为制造提交满足指标。
