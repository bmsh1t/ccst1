...
def _write_fake_trevor(path: Path, *, exit_code: int = 0) -> None:
    path.write_text(
        "\n".join(
            [
                f"#!{sys.executable}",
                "import json",
                "print(json.dumps({'user': 'alice@example.test', 'password': 'Secret#1', 'error_description': 'AADSTS50076: MFA required'}), flush=True)",
...
def test_adapter_writes_redacted_valid_jsonl_and_streams_redacted_output(tmp_path: Path):
    ...
    env, audit = _adapter_env(tmp_path, fake_trevor)
    ...
    assert rows[1]["classification"] == "unknown"
    assert rows[1]["token_issued"] is False


def test_adapter_preserves_nonzero_exit_and_records_process_exit(tmp_path: Path):
    fake_trevor = tmp_path / "trevorspray"
    _write_fake_trevor(fake_trevor, exit_code=7)
    env, audit = _adapter_env(tmp_path, fake_trevor)
    ...
    rows = [json.loads(line) for line in audit.read_text(encoding="utf-8").splitlines()]
    assert rows[-1]["event"] == "process_exit"
    assert rows[-1]["classification"] == "tool_error"
...
def test_orchestrator_routes_trevor_modes_through_adapter():
    ...
    assert 'export SPRAY_TREVOR_BIN="$(command -v trevorspray)"' in script
