...
def test_loads_source_intel_when_present(repo_root: Path) -> None:
    ...
    intel = {"routes": [{"route": "/opaque", "method": "POST", "source": "repo:app.js"}], "graphql_operations": []}
    _write(
    ...
    assert "hypotheses" not in materials["source_intel"]


def test_recovers_legacy_source_routes_without_class_predictions(repo_root: Path) -> None:
    ...
    assert result["source_intel_present"] is True
    materials = json.loads(Path(result["artifacts"]["materials"]).read_text(encoding="utf-8"))
    assert [item["route"] for item in materials["source_intel"]["routes"]] == [
        "/api/users/{id}", "/api/orders/approve",
    ]
    assert all("type" not in item for item in materials["source_intel"]["routes"])
