...
class TestRememberFinding:
    ...
    def test_host_list_relative_target_reuses_canonical_memory_identity(self, tmp_hunt_dir, tmp_path, monkeypatch):
        ...
        summary = remember_finding(
            ...
            notes="Remembered via relative list target",
        )

        assert summary["target"] == "scope.txt"
        assert summary["resolved_target"] == canonical_target
        assert summary["pattern_saved"] is True

        entries = HuntJournal(tmp_hunt_dir / "journal.jsonl").query(target=canonical_target)
        assert len(entries) == 1
        assert entries[0]["endpoint"] == "/graphql"

        profile = load_target_profile(tmp_hunt_dir, "scope.txt")
        assert profile is not None
        assert profile["target"] == canonical_target
        assert "/graphql" in profile["tested_endpoints"]
...
class TestRememberFromValidate:
    ...
    def test_from_validate_requires_context_for_repo_global_last_validate(self, tmp_hunt_dir, tmp_path, monkeypatch):
        repo_root = tmp_path / "repo"
        fallback = repo_root / "findings" / "last-validate.json"
        fallback.parent.mkdir(parents=True)
        fallback.write_text(
            json.dumps(
                {
                    ...
                    "endpoint": "https://old-target.com/api/demo",
                    "vuln_class": "idor",
                    "result": "confirmed",
                }
            ),
    ...
    def test_from_validate_rejects_explicit_target_mismatch(self, tmp_hunt_dir, tmp_path, monkeypatch):
        summary_path = tmp_path / "validation-summary.json"
        summary_path.write_text(
            json.dumps(
                {
