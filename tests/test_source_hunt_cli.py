...
def test_run_source_hunt_uses_storage_key_for_cidr_target(monkeypatch, tmp_path):
    monkeypatch.setattr(source_hunt, "BASE_DIR", tmp_path)
    ...
    monkeypatch.setattr(source_hunt, "scan_repo_secrets", lambda repo_path: [])
    monkeypatch.setattr(source_hunt, "scan_repo_ci", lambda repo_path: [])

    result = source_hunt.run_source_hunt(target="1.2.3.0/24", repo_path=str(tmp_path / "repo"))

    exposure_dir = tmp_path / "findings" / "1.2.3.0_24" / "exposure"
    assert result["status"] == "ok"
    assert exposure_dir.is_dir()
    assert json.loads((exposure_dir / "repo_source_meta.json").read_text(encoding="utf-8"))["source_kind"] == "local_path"
