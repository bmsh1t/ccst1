...
def test_red_lines_are_narrow_damage_boundaries():
    ...
    assert "唯一原则：只阻断可预见的、会伤害目标系统、真实用户或真实业务状态的破坏性动作；按实际副作用判断" in text
    assert "以上条件均不成立时直接 `allow`" in flat
...
def test_red_lines_judge_outcomes_not_request_forms():
    ...
    assert "按实际副作用判断" in text
    assert "属于红线动作" not in text
    assert "未完成红线检查" not in text
    assert "副作用不明时先做一次最小影响验证" in text
    assert "DDoS、高压流量、资源耗尽或服务中断" in text
    assert "不可控地修改、删除或污染真实数据、账号、权限、配置或业务状态" in text
    assert "`state_changing == true`" in text
    assert "测试资源上的状态变化有界、可回滚、可停止和可清理" in text
