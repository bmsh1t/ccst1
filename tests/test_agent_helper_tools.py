...
def test_dispatch_run_intel_uses_recon_tech_fallback(monkeypatch, tmp_hunt_dir, tmp_path):
    ...
    assert intel_json.is_file()
    intel_payload = json.loads(intel_json.read_text(encoding="utf-8"))
    assert intel_payload["target"] == domain
    assert intel_payload["critical"][0]["id"] == "CVE-2026-0001"
