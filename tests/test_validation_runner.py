...
def _build_case_state_for_payment_race(tmp_path):
    target = "https://target.test"
    target_case_state.add_actor(tmp_path, target, actor="owner", role="user")
    ...
    target_case_state.add_isolated_object(
        tmp_path,
        target,
        object_ref="payment_1",
        owner_actor="owner",
        endpoint=f"{target}/api/payments/1",
        cleanup_endpoint=f"{target}/api/payments/1",
...
def test_payment_race_is_blocked_without_isolated_object(monkeypatch, tmp_path):
    ...
    def fake_request_once(**kwargs):
        nonlocal called
        called = True
        return _fake_response(kwargs["url"])

    monkeypatch.setattr(validation_runner, "request_once", fake_request_once)
    summary = validation_runner.run_payment_race(
        repo_root=tmp_path,
        target="https://target.test",
        object_ref="missing-payment",
        redline_checked=True,
    )

    assert summary["result"] == "blocked"
    assert "missing" in summary["blocked_reason"]
    assert called is False
    persisted = json.loads((tmp_path / summary["summary_path"]).read_text(encoding="utf-8"))
    assert persisted["cleanup"] == {"status": "not_started"}


def test_payment_race_requires_redline_before_requests(monkeypatch, tmp_path):
    target = _build_case_state_for_payment_race(tmp_path)
    called = False
    ...
    assert summary["result"] == "blocked"
    assert "redline" in summary["blocked_reason"]
    assert called is False


def test_payment_race_caps_concurrency_and_records_cleanup(monkeypatch, tmp_path):
    target = _build_case_state_for_payment_race(tmp_path)
    lock = threading.Lock()
    calls = []
    active = 0
    ...
    def fake_request_once(**kwargs):
        ...
        try:
            if kwargs["url"].endswith("/api/payments/1") and len(calls) > 2:
                return _fake_response(kwargs["url"], status=204, body="")
            return _fake_response(kwargs["url"], body='{"status":"stable"}')
        finally:
            with lock:
                active -= 1
    ...
    assert summary["bounded_repeat"] == 2
    assert len(summary["runs"]) == 2
    assert len(calls) == 3
    assert max_active <= 2
    assert summary["cleanup"]["status"] == "completed"
    assert target_case_state.load_case_state(tmp_path, target)["objects"]["payment_1"]["status"] == "cleaned"


def test_payment_race_persists_runs_when_cleanup_fails(monkeypatch, tmp_path):
    target = _build_case_state_for_payment_race(tmp_path)

    def fake_request_once(**kwargs):
        if kwargs["url"].endswith("/api/payments/1") and kwargs["method"] == "DELETE":
            return _fake_response(kwargs["url"], status=500, body="cleanup failed")
        return _fake_response(kwargs["url"], body='{"status":"changed"}')
    ...
    assert persisted["cleanup"]["status"] == "failed"
    readiness = target_case_state.isolated_object_readiness(
        target_case_state.load_case_state(tmp_path, target), "payment_1"
    )
    assert readiness["ready"] is False
    assert "cleanup" in readiness["reason"]


def test_payment_race_rejects_single_request(monkeypatch, tmp_path):
    target = _build_case_state_for_payment_race(tmp_path)
    called = False
    ...
    assert summary["result"] == "blocked"
    assert "at least two" in summary["blocked_reason"]
    assert summary["requested_repeat"] == 1
    assert called is False
