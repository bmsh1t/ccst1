...
class TestJournalWrite:
    ...
    def test_append_auto_rotates_when_over_cap(self, journal_path, sample_journal_entry):
        journal_path.write_bytes(b"a" * 1499 + b"\n")
        journal = HuntJournal(journal_path, max_bytes=1024, keep_backups=3)

        journal.append(sample_journal_entry)

        assert (journal_path.parent / "journal.jsonl.1").exists()
        live_lines = [line for line in journal_path.read_text().splitlines() if line.strip()]
        assert len(live_lines) == 1
        assert json.loads(live_lines[0])["target"] == "target.com"
