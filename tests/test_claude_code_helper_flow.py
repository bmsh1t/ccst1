...
class TestClaudeCodeHelperFlow:
    ...
    def test_helper_chain_from_state_to_resume(self, tmp_path):
        ...
        # here (state → preflight → record → resume) are independent of which
        # of the two untested URLs ranks first.
        assert first_target["url"] in {
            "https://api.target.com/graphql",
            "https://api.target.com/api/v2/users/42/export",
        }
        ...
        # The URL chosen above (varies by value-class weighting) drives all
        # downstream assertions — extract its path and assert on that path.
        from urllib.parse import urlparse
        expected_path = urlparse(first_target["url"]).path
        assert expected_path in resume["tested_endpoints"]
        assert any(item["endpoint"] == expected_path for item in resume["findings"])
