...
def test_recon_engine_has_timeout_compat_helper():
    ...
    assert 'elif [ "$RECON_PROFILE" = "deep" ]; then' in text
    assert 'RECON_SOFT_BUDGET_SECONDS=3600' in text
    ...
    assert "BBHUNT_FFUF_RATE_LIMIT" not in text
    assert '-rate "$FFUF_RATE_LIMIT"' not in text
