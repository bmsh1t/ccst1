...
def test_read_findings_summary_prefers_scanner_summary_json(monkeypatch, tmp_path):
    domain = "target.com"
        ...
        json.dumps(
            {
                ...
                "mode": "full",
                "live_count": 3,
                "ordered_scan_count": 5,
                "skipped_checks": ["xss"],
                "categories": {
                    "sqli": {"total": 2, "files": [], "artifacts": []},
                    "mfa": {"total": 1, "files": [], "artifacts": []},
                },
                "totals": {
                    "findings": 3,
                    "manual_review_items": 4,
                    "high_value": {
                        "verified_sqli_pocs": 1,
                        "mfa_findings": 1,
                        "saml_findings": 0,
                    },
                },
                ...
                "findings": [
                    {
                        ...
                        "summary": "[SQLI-POC-VERIFIED] https://target.com/item?id=1",
                    }
                ],
            }
        ),
    ...
    assert "=== scanner_summary ===" in findings_text
    assert "mode=full" in findings_text
    assert "total_findings=3" in findings_text
    assert "manual_review=4" in findings_text
    assert "verified_sqli_pocs=1" in findings_text
    assert "categories: mfa: 1; sqli: 2" in findings_text
    assert "skipped_checks: xss" in findings_text
    assert "=== finding_index ===" in findings_text
    assert (
        "sqli_abc123 [high/confirmed] sqli https://target.com/item?id=1 "
        "status=unvalidated/not_generated"
    ) in findings_text
    assert "=== next_validation ===" in findings_text
    assert "Next validation: sqli_abc123 [high/confirmed] sqli https://target.com/item?id=1" in findings_text
    assert (
        f"python3 tools/validate.py --findings-dir {findings_dir} --finding-id sqli_abc123"
        in findings_text
    )
    assert "schema_version" not in findings_text


def test_summarize_reports_includes_report_index(monkeypatch, tmp_path):
    domain = "target.com"
    report_dir = tmp_path / "reports" / domain
    report_dir.mkdir(parents=True)
    report_file = report_dir / "sqli_001.md"
    report_file.write_text("# SQL Injection\n", encoding="utf-8")
    ...
    dispatcher = _build_dispatcher(domain, tmp_path / "agent_session.json")
    summary = dispatcher._summarize_reports(domain, 1)

    assert "generate_reports: 1 report(s) generated" in summary
    assert "Report index:" in summary
    assert "sqli_001 [high] sqli finding=sqli_abc123 file=sqli_001.md" in summary
    assert "Reports: sqli_001.md" in summary
