...
def test_web2_vuln_ab_cases_measure_incremental_skill_value():
    result = web2_vuln_ab_eval.evaluate_cases(REPO_ROOT)
    summary = result["summary"]

    assert summary["case_count"] >= 10
    assert summary["web2_skill_lines"] > 1000
    assert summary["enhanced_total"] >= summary["baseline_total"]
    assert summary["delta_total"] > 0
    assert summary["cases_with_delta"]
