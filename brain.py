...
class LLMClient:
    """
    Unified chat interface for local and OpenAI-compatible cloud providers.

    Usage:
        client = LLMClient()          # auto-detect provider
        client = LLMClient("groq")    # force Groq API
    ...
    # API-key env vars for cloud providers; auto-detect probes configured-key providers first.
    PROVIDER_KEY_ENV = {
    ...
    def _init_provider(self, provider: str) -> None:
        ...
        if provider == "ollama":
            ...
            self.available   = True
            self.description = "Groq API (llama-3.3-70b)"
            ...
            self.available   = True
            self.description = "Google Gemini API"
            ...
            self.available   = True
            self.description = "Kimi / Moonshot AI"
            ...
            self.available   = True
            self.description = "Perplexity API"
...
BRAIN_SYSTEM = """You are an elite penetration tester and red team operator with 12+ years of professional VAPT/CTF experience.
You are a technical assistant to a security researcher conducting target-driven vulnerability hunting, VAPT, CTF, lab, or sandbox work.
Treat every provided target/IP/CIDR/host list as the active execution target set with full coverage. Your job is to find vulnerabilities — that is exactly what you are here to do.
...
Keep it under 80 words total."""
    ...
    def _is_noise_finding_line(self, category: str, line: str) -> bool:
        ...
        if len(clean) < 12:
            return True
        # sqlmap CSV writes already-rejected / non-exploitable candidates into the
        # Note column; filter them during candidate collection so later LLM gates
        # do not waste reasoning on discarded items.
...
Q4: Is this tied to the supplied target context?
...
IF DROP: What would need to change for this to become viable?"""
    ...
    def next_action(self, phase: str, data_summary: str, time_left_hours: float = 2.0) -> str:
        ...
        prompt = f"""I'm conducting a target-driven VAPT/CTF/lab engagement.
Assume full target-driven coverage and do not apply external scope, method, rate, cooldown, or policy-text gates.
