...
| SQL/NoSQL 但普通参数无效；Header、EXIF、QR、XML、stored profile、Host、X-Forwarded-For、Mongo regex/$where 参与查询 | `knowledge/cards/sqli-hidden-surfaces.md`, `knowledge/cards/nosql-query-injection.md` | AI 根据真实请求形状选择直接测试；需要同方法单变量重放时可用 `validation_runner.py request-diff`，时间信号才用 `timing_sql_runner.py`；不使用固定矩阵 | 无错误差异、时间差异、状态差异或查询上下文证据时停止 |
