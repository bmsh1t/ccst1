...
| OData header/service document、`$metadata`、`$filter`、`$select`、`$orderby`、`$expand`、`$batch` | `knowledge/cards/odata-query-boundaries.md` | 保存 owner baseline；比较字段、predicate/order、navigation 和 direct-vs-batch；必要时用 `validation_runner.py idor-actor-pair` 做对象/租户对照 | metadata/operator/HTTP 200 单独保持 Signal；无受限字段、关联对象或 batch 策略差异时停止 |
