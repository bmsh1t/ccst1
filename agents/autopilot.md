...
- SQL/JSON/WAF: `wafw00f_hits.txt` is sampled host-level context, not per-request proof. AI first chooses a target-observed request shape, exact test input, and stop condition, then uses browser/MCP/curl/raw sender directly. When an exact same-method, one-dimension HTTP pair needs repeatable diff and canonical evidence, optionally call `tools/validation_runner.py request-diff`; use `tools/timing_sql_runner.py` only for a time-shaped candidate. `429`, transport failure, block pages, and WAF observations remain observations, not findings. Do not launch a fixed matrix merely because parameters or a WAF exist.
