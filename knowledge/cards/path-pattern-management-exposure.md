...
- 必要流程：目标命名规律 -> 有界词表 -> 局部递归 -> 结构化记录/真实路径反哺二次 recon。
- 可复用经验：目标特定词表通常优于通用目录字典；结构化记录、接口统计、访问日志、配置字段常能暴露更高价值路径、参数和 secret 候选。
...
- 登录页使用默认品牌、缺少验证码/锁定提示、错误信息稳定；这可以形成受控口令测试 lead，但不得由本卡或 `/autopilot` 自动进入爆破。
...
- 管理/监控/日志/统计/配置入口只先做安全识别：标题、版本、认证要求、只读页面、结构化数据、配置字段；口令测试作为手动 `/spray` 或 `credential-attack` 后续动作。
...
- 入口需要口令爆破、默认凭据检查或真实账号尝试时，当前发现 lane 停止并转手动 `/spray` / `credential-attack`；验证码绕过、账号锁定规避或大规模猜测按红线处理。
