...
## ID 形态枚举决策（蒸馏自 Skills-v4 predictable-identifiers，2026-09-14）

枚举预算花不花由 ID 形态的判定树决定，不是看到非自增 ID 就放弃：

- 收集 10+ 个 ID 做位级分解，识别时间戳段/机器段/序列段/随机段。
- 时间戳+序列占主导（UUIDv1 第 13 段为 1、雪花 41+10+12 位、ULID 前 10 字符
  是 Base32 毫秒时间戳）→ 已知一个 ID 即可推相邻 ID，枚举值得做。
- 随机段非密码学随机（java.util.Random 48 位种子、mt_rand、时间戳作种子）
  → 观察序列反推 RNG 状态后预测，先推状态再谈枚举。
- 随机段 ≥64 位且密码学随机 → 枚举是死路，价值判定转回对象授权差本身
  （ID 不可猜 ≠ 无 IDOR；owner/peer 替换矩阵仍然适用）。
- Hashids/Base62 → 先试默认 salt 解码，自定义 salt 才评估爆破成本。
