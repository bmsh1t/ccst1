...
- 停止：当前已测边界由服务端稳定拒绝且无相反证据、没有可复现请求，或继续操作会影响真实账号/业务状态。
...
- 单个接口拒绝、不再回显或旧验证步骤失效，只否定该 method/path/actor/object/state 下的观察，
  不自动证明 token 已失效、相邻入口已修复或整条权限链路关闭。
- 只有当前证据仍显示 token/session 被接受、相邻受保护接口可达或身份状态矛盾时，才选择一个
  已观察到的相关入口继续最小验证；没有新变量时按原停止条件收敛，不展开通用接口清单。
- 使用自有/测试账号的独立会话，排除已有高权限 cookie、缓存和前端展示；在同一受保护请求上
  对照无 token/无效 token 与待验证 token，并保留后端身份、对象或权限差异的脱敏证据。
- token 可解析、返回普通身份信息或显示管理员菜单/水印，本身不证明权限缺陷；先确认目标的
  撤销/权限契约，再证明服务端实际放行不应允许的行为。不能确认时保留 Signal/Lead 或明确 blocked。
- 原阴性观察与新的相反证据分别绑定既有 evidence refs；后续对照证伪残余能力时，只关闭相应
  假设，不抹掉原观察，也不把本轮结果扩大成全目标安全结论。
