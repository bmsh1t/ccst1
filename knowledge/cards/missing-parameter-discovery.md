...
- 必须保留：缺参/校验错误只能作为信号，不是漏洞结论；命中后仍需最小影响验证和红线检查。
- 必要流程：错误信号 baseline -> 目标特定参数词表 -> 低频收敛 -> 单参数响应形态差异 -> 最小影响验证。
- 可复用经验：后端错误、校验提示、响应长度/结构变化能反推出隐藏输入面；目标自身词表通常优于通用参数字典。
- 不沉淀为默认动作：批量枚举真实 PII、凭据、联系方式、地址、token 或业务数据，不保存完整真实数据集，不把某个具体参数名、接口名、框架路径或工具绑定为必选。
...
- 分组/差异收敛：用低频分组探测或二分收敛减少请求量；Arjun 只是其中一种实现，不是唯一工具。
...
- 不吸收案例里的具体参数名、接口名、路径绕法或框架字典作为固定流程。
- 不吸收批量遍历真实 PII、凭据、联系方式、地址或业务数据这类扩大影响动作。
- 不把 Burp、Excel、Arjun 等案例工具固定为必用；核心是“目标词表 + 差异收敛 + 最小验证”。
