...
知识库是 Skills 的回忆和联想层：负责提供可复用模式、反例、发散问题和思路变形，不负责指挥流程。具体执行顺序、工具选择、验证深度和写回位置仍由当前 Skill 与检查层决定。
...
5. 知识卡产出的新思路必须交还给当前 Skill 决策，并回到目标层记录为 lead / next action / dead end。
