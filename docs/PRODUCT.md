...
- 大型 URL 语料先构建 exact URL/provenance 索引：只合并完全相同的 URL，参数值与顺序、
  duplicate key、编码、scheme/port、path 大小写和尾斜线等 variant 全部保留；shape 只用于
  统计、导航和长尾分页
- 完整 surface refresh 对每个唯一、target-owned URL 逐条评分，再维护有界 P1/P2/Review
  frontiers；首屏/top-K 是注意力窗口，不是攻击面截断或 coverage 结论
...
state。compact builder 只读取 control facts、文件 stat、inventory summary 和 exact-hit bounded
surface projection；不扫描大型 `with_params.txt`、不解析 monolithic `observations.json`、不执行
ranking，也不写 target state。projection 缺失、过期或损坏时返回
`prepare_surface_context`，不会把未知攻击面误报为 clean/exhausted。默认顺序执行，2-8 GB
只是已验证的常见资源档位而非产品硬门；流程不会隐式并行或启动第二个 controller。正式语法为
`/autopilot <target> [--paranoid|--normal|--yolo] [--quick] [--deep [--max-lanes N]] [--auth-file PATH]`。
`--deep` 默认仍按目标价值持续深入；显式 `--max-lanes N` 只限制当前 invocation 的命名实质
lane 数，达到 N 后 checkpoint 会同步 durable queue、给出 handoff 并结束本次会话，剩余/new
surface 留给下一次 `/autopilot`，不是目标全局 coverage 上限。
recon 成功收尾会非致命执行一次共享 surface finalizer，生成完整 exact index、inventory summary
和 bounded projection。若派生步骤失败，原始 recon 仍保留，下一次 state 明确要求
`python3 tools/surface.py --target <target> --refresh` 恢复；bootstrap、完整 state 和 context-pack 在同一 fingerprint 下复用投影，
避免重复 ranking。
...
| `tools/surface.py` | 对完整 exact target-owned URL 流评分并生成 scanner / intel / js_intel / browser / memory 的 bounded advisory review pack |
| `tools/surface_index.py` | 外部排序的 exact URL/provenance 索引、无损 variant/shape 统计和 cursor 分页 |
| `tools/surface_projection.py` | 绑定全部排序输入的 bounded surface cache；只有 exact fingerprint hit 可消费 |
| `tools/surface_finalizer.py` | recon 收尾共用的非致命 index/rank/inventory-summary/projection 派生器 |
...
├── params/          参数发现结果
├── exposure/        配置文件暴露结果
├── surface/         可删除重建的 exact URL 索引、manifest 与聚合 summary
...
| `recon/<target>/surface/` | 可删除重建的 exact URL/provenance index、输入 manifest 与 shape/variant 聚合 |
| `state/<target>/observations.json` | 完整中性 observation inventory；首屏之外的条目仍保留原 lifecycle |
| `state/<target>/observations-summary.json` | 与 inventory/source 绑定的轻量计数和 bounded sample，不是第二真相 |
| `state/<target>/surface-projection.json` | manifest-bound bounded 排名投影；stale/missing 不参与 next-action 判断 |
