...
- Every substantive candidate is claimed before replay with
  `python3 tools/action_queue.py claim --target <target_shell> --id <id> [--template <name>] [--from-evidence <probe-or-evidence-json>] --metadata-json '<activation-object>'`.
  `--template`（`--list-templates` 列举）预填类别稳定字段（family/technique/
  dimension/skill_route/risk_tier），AI 的 `--metadata-json` 显式值永远覆盖模板；
  四个判断字段（hypothesis_id/expected_learning/kill_condition/decision_reason）
  不在任何模板里，仍必须显式给出。
