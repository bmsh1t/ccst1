...
- Every substantive candidate is claimed before replay with
  `python3 tools/action_queue.py claim --target <target_shell> --id <id> [--from-evidence <probe-or-evidence-json>] --metadata-json '<activation-object>'`.
  family/technique/skill_route/risk_tier 由 AI 按当前假设填写（类别模板已退役，
  2026-09-13 原生能力审计：预设类别值复制的是判断，不是格式）；四个判断字段
  （hypothesis_id/expected_learning/kill_condition/decision_reason）仍必须显式给出。
  ...
  describes the `skill_route` shape (`skill_path` is derived from `skill_id`,
  never hand-written), target-owned evidence refs, optional knowledge
  ...
  guessing/retry; do not retry an incomplete object. `--from-evidence` only
  derives the mechanical fields (endpoint, method, evidence_ref, baseline_ref)
  from a target-owned probe/evidence JSON; explicitly supplied `--metadata-json`
  values override derived values and judgment fields are never derived — the
  full 16-field depth contract still applies.
...
- `resume_action_queue`: run `python3 tools/action_queue.py claim --target <target_shell>`, perform the claimed or resumed durable replay, then run `python3 tools/action_queue.py resolve --target <target_shell> --id <id> --status <state> --evidence <why>` and refresh. For continuation resolves, submit the four-field continuation (`kind`/`dimension`/`question`/`expected_learning`/`reason`) directly in the resolve metadata — AI writes it from the current hypothesis per the published Queue schema; the blank-skeleton generator is retired.
