...
|---|---|---|
| IDOR on GET `/api/user/X/orders` | Read-only sibling exposure only | Record order lifecycle write actions; do not invoke |
...
| Race condition | Non-payment quota/rate-limit surfaces | Race on rate limits |
...
3. Before reporting, B must pass Gate 0 independently or the full A→B path must be proven end-to-end
4. Never report A + B as one report unless they ARE one proven attack chain
