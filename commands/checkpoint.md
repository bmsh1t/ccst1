...
默认不写 target memory，但会刷新派生 coverage、通过 `action_queue` owner 幂等同步
可执行 next-action，并原子更新 `state/<target_key>/checkpoint_latest.json` runtime-v2 witness。
只有用户明确要求时，才使用 `--apply-target-memory` 写入目标记忆层。
