...
def resolve_output_path(*, target: str = "", output: str = "", repo_root: str | None = None) -> str:
    """Resolve the learn/intel output path using canonical target storage keys."""
    if output:
        return output

    resolved_repo_root = repo_root or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    if target:
        base_dir = os.path.join(resolved_repo_root, "recon", target_storage_key(target))
        os.makedirs(base_dir, exist_ok=True)
        return os.path.join(base_dir, "intel.md")

    return os.path.join(resolved_repo_root, "intel.md")
...
def main():
    ...
    output_path = resolve_output_path(target=args.target, output=args.output)
