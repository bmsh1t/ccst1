#!/bin/bash
# =============================================================================
# Spray Orchestrator — manual password spray with typed-hostname + lockout warn
#
# Modes:
...
# Hard guard 1: operational risk reminder + typed-hostname confirmation
# The real safety mechanism here is making the operator re-state the target
# host out loud before any live authentication attempt.
echo ""
log_warn "${BOLD}OPERATIONAL RISK REMINDER${NC}"
log_warn "This sends live authentication attempts and can trigger lockouts,"
log_warn "rate limits, alerts, or noisy account-state changes. Verify NOW:"
log_warn "  - Target host and login endpoint are exactly the ones you intended"
log_warn "  - Users/password files are intentionally selected and small enough"
log_warn "  - Success/failure regexes are configured when HTTP heuristics are weak"
log_warn "  - If unsure, ABORT and run with --dry-run first"
...
log_warn "Locked accounts may trigger alerts and disrupt follow-up testing."
