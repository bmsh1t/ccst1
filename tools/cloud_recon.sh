...
# 与 Osmedeus workflows/common/01-osint.yaml 的 toolsDir 约定保持一致：
# 优先复用 $HOME/Tools，可用 BBHUNT_TOOLS_DIR / OSMEDEUS_TOOLS_DIR 覆盖。
...
run_cloud_enum() {
  local keyword="$1"
  local output="$2"
  local cloud_enum_bin cloud_enum_script cloud_enum_py
  ...
  if [ -n "$cloud_enum_bin" ]; then
    "$cloud_enum_bin" -k "$keyword" -t 5 -qs -l "$output"
  elif [ -f "$cloud_enum_script" ]; then
    [ -x "$cloud_enum_py" ] || cloud_enum_py="python3"
    "$cloud_enum_py" "$cloud_enum_script" -k "$keyword" -t 5 -qs -l "$output"
  else
    return 127
  fi
}
...
if [ -n "$KEYWORD" ] && [ "$S3_ONLY" = "0" ]; then
  ...
  if run_cloud_enum "$KEYWORD" "$OUT_DIR/cloud_enum.txt" 2>/dev/null; then
    n=$(wc -l < "$OUT_DIR/cloud_enum.txt" 2>/dev/null | tr -d ' ' || echo 0)
    [ "$n" -gt 0 ] && hit "cloud_enum: $n discoveries — review file" || ok "cloud_enum: clean"
  else
    rc=$?
    if [ "$rc" -eq 127 ]; then
      warn "cloud_enum not found in PATH or $SHARED_TOOLS_DIR/cloud_enum — skipping"
    else
      warn "cloud_enum exited non-zero — review environment/network and rerun if needed"
    fi
  fi
fi
