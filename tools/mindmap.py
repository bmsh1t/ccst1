...
def resolve_output_path(*, target: str, output: str = "", repo_root: str | None = None) -> str:
    """Resolve the default mindmap output path using canonical storage keys."""
    if output:
        return output

    resolved_repo_root = repo_root or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    base_dir = os.path.join(resolved_repo_root, "findings", target_storage_key(target))
    os.makedirs(base_dir, exist_ok=True)
    return os.path.join(base_dir, "mindmap.md")
...
def main():
    ...
    output_path = resolve_output_path(target=target, output=args.output)
