...
def _hypothesis_seeds(cards: list[str], blob: str, local_intel: dict) -> list[str]:
    ...
    if CARD_PATHS["proxy-cache-boundaries"] in cards:
        seeds.extend([
            ...
            "Web cache deception 要从自有账号私有页 baseline 开始，分别测试 path mapping、delimiter、origin/cache normalization 和 exact-match file rule；smuggling-to-WCD 还要检查 incomplete-header inner request 是否继承 victim Cookie 并把私有页错配到 JS/CSS/image cache key；投递 victim 前换唯一 URL，最后用 no-cookie/raw read 证明私有响应被共享缓存。",
