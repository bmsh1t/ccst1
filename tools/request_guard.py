#!/usr/bin/env python3
"""
request_guard.py — request telemetry helper for Claude Code autopilot loops.

Historical name note: this module no longer acts as an execution guard. It
adds a thin, file-backed advisory layer around active requests:
- preflight: scope/method/breaker/rate telemetry without hard blocking
...
def _advisory_payload(
    *,
    ...
    suggested_wait_seconds: float = 0.0,
) -> dict:
    payload = {
        "allowed": True,
        "action": action,
        ...
        "scope": scope_info,
        "wait_seconds": 0.0,
        "suggested_wait_seconds": round(max(0.0, suggested_wait_seconds), 3),
        "interval_seconds": round(max(0.0, interval_seconds), 3),
        "settings": settings,
    ...
    if advisories:
        ...
        payload["reason"] = "; ".join(advisories)
    return payload
...
def preflight_request(
    *,
    ...
    """Run request preflight telemetry and return advisory-only guidance."""
    ...
    if remaining > 0:
        advisories.append(f"circuit breaker advisory active for {remaining:.3f}s")

    interval = _interval_seconds(settings, is_recon=is_recon)
    suggested_wait_seconds = round(
        max(0.0, host_state.get("last_request_ts", 0.0) + interval - current_ts),
        3,
    )
    if suggested_wait_seconds > 0:
        advisories.append(f"rate-limit advisory suggests waiting {suggested_wait_seconds:.3f}s")

    host_state["last_request_ts"] = float(current_ts)
    state["hosts"][host] = host_state
    save_guard_state(memory_dir, target, state)

    action = "allow"
    if advisories:
        action = f"{action}_advisory" if action != "allow" else "allow_advisory"
...
def format_guard_output(payload: dict, command: str) -> str:
    ...
    if command == "preflight":
        suggested_wait = float(payload.get("suggested_wait_seconds", payload.get("wait_seconds", 0.0)) or 0.0)
        summary = (
            f"ALLOW {payload['method']} {payload['url']}\n"
            f"Suggested wait: {suggested_wait:.3f}s\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Request telemetry helper for autopilot loops (historical name: request_guard)"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    preflight_parser = subparsers.add_parser("preflight", help="Record advisory preflight telemetry before a request")
